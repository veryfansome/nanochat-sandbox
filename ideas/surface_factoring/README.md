...
**Status (2026-06-25): overlay + harness built; first model A/B in; CORE still the gate.** The tokenizer-level compression and lossless rendering are measured (see [Results](#results)) — the full triple tokenizer (force_merges + space + case) compresses **+11.11%** vs baseline. The model overlay + training harness are now **built and smoke-validated** (25 checks across 4 suites), and the first d6 model A/B is in: surface factoring posts the **lowest val bpb of any d6 run** (1.1378, −0.028 vs baseline / −0.021 vs force_merges, same nanochat commit — see [Downstream](#downstream-runcpu-d6--the-first-model-ab)). What remains is the **surface-aware eval/generation path** (CORE joint-likelihood scoring + `base_eval`/generate + checkpoint-load via persisted `K_max`), then the GPU A/B — the gate for the *capability* verdict, since bpb ≠ CORE. All offline numbers reproduce with no GPU via the [scripts](#scripts).
...
**Retired vs. remaining.** The *correctness / "do the pieces fit"* risk is retired at every level: contract (rungs 0–4 on the tiny reference), real-integration (`overlay/surface_factoring.py` bit-identical to vanilla GPT with surface off), and **end-to-end harness** (the d6 runcpu trained cleanly to the lowest bpb of any d6 run — see [Downstream](#downstream-runcpu-d6--the-first-model-ab)). **Built + validated (25 checks across 4 suites):** the overlay (surface-conditioned value embeds + heads + train/eval loss), the codec + `SurfaceTokenizer`, the harness (`wrappers/train_surface_factoring.py` + `overlay/surface_dataloader.py` + `surface_context.py` + `wrappers/tok_train_surface.py` + `runs/runcpu_surface.sh`). The production triple tokenizer is **cache-generated on demand** by `tok_train_surface` (not a committed artifact). **Remaining before the GPU A/B:** the **surface-aware eval/generation path** — `core_eval` joint-likelihood scoring, surface-aware `base_eval`/`chat_*` (these still build vanilla GPT + strict-load), generation with context-derived masks, and checkpoint-load via persisted `K_max`. Plus the [throughput](#training-integration) follow-ups for a fast multi-GPU run (batch-encode / background-prefetch). One known generation cost stands: occurrence-active masks must be derived from running context at generation (the eval smoke approximates the *input* mask; decode recomputes ownership exactly, so correctness holds).
...
1. **Production tokenizer wrapper — ✅ BUILT.** `overlay/surface_codec.py` (the unicode-safe dual-stream `surface_encode`/`surface_decode`/`compute_kmax`, tokenizer-agnostic) + `overlay/surface_tokenizer.py` (`SurfaceTokenizer`: `from_directory` load, `encode`/`decode`, `byte_count` for the bpb denominator). Regression-tested in `wrappers/smoke_surface_factoring.py` (`check_surface_tokenizer`). The production-vocab (32890) triple tokenizer artifact is **cache-generated on demand** by `wrappers/tok_train_surface.py` (folded corpus + spaceless pattern + folded forced pairs → saved to `~/.cache/nanochat-variants/surface_triple/tokenizer/` with `token_bytes.pt`); `runs/runcpu_surface.sh` runs it before training. It is reproducible from the corpus, not a committed/prebuilt file — so a fresh machine regenerates it (a few minutes) rather than downloading it.
