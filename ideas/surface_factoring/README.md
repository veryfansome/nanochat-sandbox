...
**Retired vs. remaining.** Rungs 0–4 retire the *correctness / "do the pieces fit"* risk — the dual-stream contract (incl. unicode), the byte-denominator fix, the slot-specific injective cap embedding, the occurrence-active mask, the param-accounting overrides, reduces-to-baseline, that the heads learn, and that the eval/generation paths run and rank sanely. Still open: **rung 5** (`runcpu.sh`, d6 real loop — mechanism verification at scale) and the **Lambda A/B** (quality — does CORE/bpb actually improve), neither run yet. **Caveats on the smokes:** they exercise a *tiny reference*, not the production overlay (which still needs the real `GPT.forward` integration, the `value_embeds` decision, the checkpoint metadata persistence + surface-aware load/eval/SFT/chat wrappers, and the production O(tokens) encoder with the validated lone-space rule). The reference encoder/decoder IS unicode-safe (round-trips `Łódź`/`Ελληνικά` and operates on the reconstructed string for byte-split safety); the one generation cost it surfaces is that occurrence-active masks must be derived from running context at generation time (the smoke uses an intrinsic-word-start approximation for the input mask — decode recomputes ownership exactly).
...
- **Downstream (CORE/bpb) is UNVERIFIED.** Everything here is offline tokenizer compression + lossless rendering. force_merges itself is downstream-validated (CORE +5.81%); the open question is whether the surface-factoring increment adds CORE on top.
...
Decision points still open: which force_merges phrase set (full 122 → +11.11%, or 69 word-phrases-only → cleaner tokens, +6.56% before case); whether to ship space+case or start with space-only to isolate; and the exact sequential-conditioning mechanism for the surface head.
