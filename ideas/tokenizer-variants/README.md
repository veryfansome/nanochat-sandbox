...
6. **Re-engage variant 6 (seed tokens)** from `seed_tokens` — interesting, less validated; standalone Lambda A/B is the next step. The force_merges adoption validated the data-driven mining methodology, so the same workflow (offline eval → Lambda speedrun) applies.
