...
- For forced merges: which phrases to force. **Resolved by data-driven curation (2026-05-25).** Replaced the ~70-pair hand-picked list with `tools/mine_forced_pairs.py --two-pass` + `tools/ablate_derived.py`; final list at [`rustbpe_variants/force_merges/pairs.py`](../../rustbpe_variants/force_merges/pairs.py) (87 mined FORCED + 15 mined DERIVED + 2 numeric DERIVED = 104). Achieves **−6.5% total tokens vs the baseline nanochat tokenizer** on a 15M-char training-corpus sample. Full construction provenance + per-pair attribution table + vocab-cost mechanism + reusable infrastructure list lives in [`STATUS.md`](../../STATUS.md) under the `force_merges` variant entry. To re-run the curation against a different corpus, regenerate the mining artifacts at `rustbpe_variants/force_merges/mined/` and rerun `ablate_derived` to find the new K cutoff.
