...
5. **Variant 7 (forced merges)** — **Done** via data-driven curation (`tools/mine_forced_pairs.py --two-pass` + `tools/ablate_derived.py`); see the "For forced merges" item under Open questions below for the resolved state and STATUS.md §force_merges for the full curation details. Currently in a Lambda speedrun vs `d24_baseline`.
6. **Re-engage variant 6 (seed tokens)** from `seed_tokens` — interesting, less validated; do after the force_merges Lambda outcome arrives.
7. **Variants 4 (vocab grid), 8 (min_frequency), 9 (vocab pruning)** — narrower questions, do as targeted follow-ups.
8. **Variant 13 (case-marker)** — pursue if either (a) the force_merges Lambda speedrun shows a clear val/bpb win AND we want to chase more lossless-compression sources, or (b) we want a more disciplined version of the "lowercase everything" intuition without giving up val/bpb-comparability or CORE eval. Combines orthogonally with whichever variants land first.
...
- For forced merges: which phrases to force. **Resolved by data-driven curation (2026-05-25).** Replaced the ~70-pair hand-picked list with `tools/mine_forced_pairs.py --two-pass` + `tools/ablate_derived.py`; final list at [`rustbpe_variants/force_merges/pairs.py`](../../rustbpe_variants/force_merges/pairs.py) (87 mined FORCED + 15 mined DERIVED + 2 numeric DERIVED = 104). Achieves **−6.5% total tokens vs the baseline nanochat tokenizer** on a 15M-char training-corpus sample. Full construction provenance, per-pair attribution table, vocab-cost bracketing experiment, known issues (shadow-detection over-conservatism), and reusable infrastructure live in [`STATUS.md`](../../STATUS.md) §force_merges. To re-curate against a different corpus: (1) regenerate the mining artifacts under `rustbpe_variants/force_merges/mined/`, (2) rerun `ablate_derived` to find the new K cutoff, (3) **paste the new pair tuples into `pairs.py`** (it's intentionally a flat literal list, not a runtime loader — see the module docstring for the regen command), (4) **re-validate the `_NUMERIC_DERIVED` hand-adds** (`("00","0")`, `(",","000")`) against the new corpus + probe battery — they were originally added because the `currency` probe regressed without them, but a different corpus may need different numeric concats (or none).
