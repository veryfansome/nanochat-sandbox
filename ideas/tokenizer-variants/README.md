...
**Resolution in this project (2026-05-26)**: we ported this approach (constructive merge-chain builder, YAML morpheme list) into `rustbpe_variants/seed_tokens/` and tested it on the held-out ClimbMix val corpus. The morpheme-mining approach was **net-negative at every K threshold tested** — the surfaced morphemes (`cation`, `tation`, `zation`, `lization`, ...) competed with baseline's already-efficient short morphemes (`ing`, `ation`, `ate`, `ize`) rather than complementing them. See STATUS.md §seed_tokens redesign for the full diagnostic. We then reframed the mining target to **within-pre-tok-chunk adjacent-token-pair bigrams in baseline output** (`tools/mine_seed_compositions.py`) — pairs baseline produces as adjacent but couldn't merge under the 32K budget. That approach was net-positive (−0.63% ClimbMix val tokens at +5.3% vocab, K=1750 canonical). The original Rust crate + YAML are preserved in-tree as history but no longer referenced; the current canonical is a pure-Python overlay (`wrappers/tok_train_seed_tokens.py` + `rustbpe_variants/seed_tokens/routes.py`).
...
6. **Seeded-merge BPE** — **(engaged 2026-05-26; canonical adopted as within-chunk composition mining)**. Two iterations:
   - **First**: ported `origin/seed_tokens`'s YAML morpheme list + Rust constructive-merge builder. **Net-negative on ClimbMix val** at every K threshold tested — competed with baseline's already-efficient short morphemes. The original Rust crate + YAML preserved in-tree for history; `tools/mine_seed_morphemes.py` + `tools/find_seed_routes.py` likewise preserved.
   - **Second (canonical)**: reframed the mining target to **within-pre-tok-chunk adjacent-token-pair bigrams in baseline output** (`tools/mine_seed_compositions.py`). Each pair is a composition baseline knows is common but couldn't fit in the 32K budget; the pair IS the route, since both operands are baseline tokens by construction. The wrapper inserts each seed at rank `max(id_L, id_R) + 1` (mid-training-style) in pure Python — no Rust crate needed. Canonical: K=1750 at the per-seed-marginal plateau-end of the K-sweep curve. Offline result: **−0.63% ClimbMix val tokens at +5.3% vocab**, Lambda-pending. See STATUS.md §seed_tokens redesign.
...
6. **Variant 6 (seeded merges)** — **Done offline; Lambda-pending**. Morpheme-mining iteration was net-negative; within-chunk composition-mining iteration is net-positive (−0.63% ClimbMix val tokens at +5.3% vocab, K=1750 canonical). Smaller offline magnitude than force_merges's −6.9%; uncertain whether it crosses the threshold for a real CORE/val-bpb win at d24 scale. Two open paths before a Lambda commit: (a) offline A/B with `force_merges` stacked (orthogonal gaps), (b) direct standalone d24 Lambda A/B (~$120).
...
- For seed tokens: **resolved by data-driven within-chunk composition mining (2026-05-26)**. Replaced the intuition-driven `seed_tokens.yaml` morpheme list with `tools/mine_seed_compositions.py` (adjacent-token-pair bigrams in baseline ClimbMix val output, filtered to concats not in baseline vocab, ranked by count). K=1750 canonical at the plateau-end of the per-seed-marginal K-sweep curve. The morpheme-mining variant is preserved as deprecated; see STATUS.md §seed_tokens redesign for the full empirical arc and the per-token usage-delta diagnostic that drove the reframing.
- Whether to combine forced merges + seed tokens — they target **different structural gaps**: `force_merges` captures cross-pre-tok-chunk pairs (BPE structurally cannot see them; addressed via regex carve-out), while `seed_tokens` captures within-chunk pairs that BPE saw at training time but couldn't fit in the 32K budget (addressed via mid-rank insertion). Mechanically orthogonal (no shared mechanism that could interfere). Untested as a stack; an offline A/B before any Lambda commit is the cheap next step.
