...
| [Surface factoring](surface_factoring/README.md) — factor space + capitalization out of token identity into side-channel bits | sample efficiency (reclaim ~24% vocab duplication → denser merges). **Surface-only (space+case on the baseline vocab, 32768, K_max=1, no force_merges): offline +4.90% tokens; no model A/B yet — the clean factoring-isolation test (surface-only vs baseline) is the open question. The earlier fm+space+case TRIPLE (+11.11%, K_max=3) was a premature stacking — DEPRECATED, its d24 A/B (surface TIES fm) a dead-end that confounded the read; checkpoint abandoned. Untested levers: λ=0.5, exposure/scale, lighter value-embeds. See [the idea doc](surface_factoring/README.md).** | preprocessing artifact + model overlay + harness change (tokenizer build + joint surface head + bit targets in the dataloader/loss) |
