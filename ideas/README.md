...
| [Surface factoring](surface_factoring/README.md) — factor space + capitalization out of token identity into side-channel bits | sample efficiency (reclaim ~24% vocab duplication → denser merges). **Offline-validated: triple stack force_merges+space+case = +11.11% tokens vs baseline, lossless; downstream A/B pending.** | preprocessing artifact + model overlay + harness change (tokenizer build + joint surface head + bit targets in the dataloader/loss) |
