...
| [Surface factoring](surface_factoring/README.md) — factor space + capitalization out of token identity into side-channel bits | sample efficiency (reclaim ~24% vocab duplication → denser merges). **Offline +11.11% tokens; overlay + harness BUILT (25 smoke checks); first d6 runcpu A/B = lowest bpb of any d6 run (1.1378, −0.028 vs baseline / −0.021 vs force_merges, same commit); CORE pending the surface-aware eval path.** | preprocessing artifact + model overlay + harness change (tokenizer build + joint surface head + bit targets in the dataloader/loss) |
