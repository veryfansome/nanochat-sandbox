...
| [Surface factoring](surface_factoring/README.md) — factor space + capitalization out of token identity into side-channel bits | sample efficiency (reclaim ~24% vocab duplication → denser merges). **Surface-only (space+case on the baseline vocab, 32768, K_max=1): offline +4.90% tokens. PAIRED d24 A/Bs (λ0.1) + verification controls (2026-06-28): NOT a capability win** — s7 ClimbMix CORE +0.0072 (0.2744 vs 0.2673), s8 FineWeb CORE +0.0111 (0.2436 vs 0.2325), val_bpb slightly worse; the leans decompose mostly to artifacts (commonsense_qa letter-prior, copa small-N, boolq calibration; arc content-scored + positive both seeds, its own control TBD). Robust surface effects = the by-design factored-axis bpb discounts only (cap-bit/case + small pooling). Separately, a cross-seed CORPUS effect (vanilla ClimbMix−FineWeb +0.0347 CORE) is mostly real reading in the content-MC channel (premise-swap). The earlier λ=0.5 WASH + λ-screen HOLD + fm+space+case TRIPLE are superseded/deprecated. See [the idea doc](surface_factoring/README.md).** | preprocessing artifact + model overlay + harness change (tokenizer build + joint surface head + bit targets in the dataloader/loss) |
