...
| [Surface factoring](surface_factoring/README.md) — factor space + capitalization out of token identity into side-channel bits | sample efficiency (reclaim ~24% vocab duplication → denser merges). **Surface-only (space+case on the baseline vocab, 32768, K_max=1): offline +4.90% tokens. d24 A/B DONE (2026-06-26): a WASH — CORE ties vanilla baseline (0.2654 vs 0.2604, 10W/10L/2T) with val_bpb slightly worse, and below force_merges (0.2758); the +4.90% compression → no capability/bpb gain (compression ≠ capability at d24/λ=0.5/single-seed). The earlier fm+space+case TRIPLE (+11.11%, K_max=3) A/B (surface ties fm) is separate DEPRECATED history. λ-screen DONE (2026-06-27): HOLD — λ=0.5→0.1 has no detectable MC-trio effect (Δ−0.0006). Open: a single-task boolq swing (+266Q, needs a probe/2nd seed). See [the idea doc](surface_factoring/README.md).** | preprocessing artifact + model overlay + harness change (tokenizer build + joint surface head + bit targets in the dataloader/loss) |
