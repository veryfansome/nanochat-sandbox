...
force_merges appends-at-end and needs a carve-out because it forces *cross-boundary* pairs BPE can't otherwise form — unlike the earlier within-chunk forcing variants (`seed_tokens`, `manual_merges`, both removed 2026-06-05), which used the standard regex and inserted within-chunk pairs at natural rank.
...
Data-driven curation tooling (`tools/mine_forced_pairs.py`, `tools/ablate_{mined,derived}.py`, the `tok_train_force_merges_mined_*` subset trainers) was **removed 2026-06-05** now that force_merges is frozen as a kept result — recoverable from git; the procedure is preserved in [`../../STATUS.md`](../../STATUS.md) §force_merges.
