"""
One-shot offline report for a blocked-pairs tokenizer variant (auto_tune). Run it
after each retrain to get, in a single command:
...
Usage:
    # defaults: auto_tune variant vs baseline, dumps to /tmp, 20M-char corpus
    uv run python -m tools.blocked_pairs_report

    # any other blocked-pairs variant
    uv run python -m tools.blocked_pairs_report \\
        --variant ~/.cache/nanochat-variants/<variant>/tokenizer \\
        --pairs-module rustbpe_variants.<variant>.pairs
...
def main():
    ...
    p.add_argument(
        "--variant",
        default=os.path.expanduser("~/.cache/nanochat-variants/auto_tune/tokenizer"),
        help="Variant tokenizer dir (default: auto_tune).",
    )
    p.add_argument(
        "--baseline",
    ...
    p.add_argument(
        "--pairs-module",
        default="rustbpe_variants.auto_tune.pairs",
        help="Importable module exposing BLOCKED_PAIRS (default: auto_tune).",
    )
