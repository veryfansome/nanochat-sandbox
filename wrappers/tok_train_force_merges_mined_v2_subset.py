"""
DERIVED-pair ablation tokenizer training.

Trains a force_merges_mined_v2 tokenizer with the mined DERIVED list truncated
to the top-K entries (DERIVED_TOP_K env var). The 2 numeric DERIVED + the
full mined FORCED list are always included. Writes to a per-K variant base
dir so multiple K values can coexist for side-by-side eval.

Output dir: ~/.cache/nanochat-variants/force_merges_mined_v2_d${DERIVED_TOP_K}/tokenizer/
"""
import os
import runpy
...
def apply_patches():
    ...
    from rustbpe_variants.force_merges.pairs_mined_v2_subset import (
        SPLIT_PATTERN,
        FORCED_PAIRS,
        BLOCKED_PAIRS,
    )
    ...
    def _patched_train_classmethod(text_iterator, vocab_size):
        ...
        enc = tiktoken.Encoding(
            name="rustbpe_force_merges_mined_v2_subset",
            pat_str=pattern,
...
def main():
    ...
    variant_base = setup_variant_base(f"force_merges_mined_v2_d{k}")
