"""
Trainer for the FORCED-list subset. Reads `MINED_TOP_K` (default: all),
trains a tokenizer with that many top-frequency FORCED pairs, writes to a
K-specific dir so K values coexist. Invoked by `tools/ablate_mined.py`.

Output: ~/.cache/nanochat-variants/force_merges_mined_k${MINED_TOP_K}/tokenizer/
"""
import os
import runpy
...
def apply_patches():
    ...
    from rustbpe_variants.force_merges.pairs_mined_forced_subset import (
        SPLIT_PATTERN,
        FORCED_PAIRS,
        BLOCKED_PAIRS,
    )
