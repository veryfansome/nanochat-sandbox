...
| [Surface factoring](ideas/surface_factoring/README.md) | ✓ | **surface-only** (space+case on the baseline vocab, 32768, K_max=1, no force_merges) BUILT: offline tools (`tools/{space_factor_compression,case_factor_compression,case_factor_analysis}.py`) + overlay/harness `overlay/surface_{factoring,codec,tokenizer,dataloader,context,lambda}.py` + `wrappers/{train_surface_factoring,tok_train_surface}.py` + `runs/runcpu_surface.sh`; 29 smoke checks (`wrappers/smoke_surface_factoring{,_eval,_overlay,_harness,_core}.py`). (`tools/stack_triple` builds the DEPRECATED triple — kept for reference only.) | A/B **DONE** (wash) + **λ-screen DONE (HOLD)** + **s7+s8 PAIRED A/Bs + verification controls DONE (2026-06-28): NOT a capability win** | surface-only offline **+4.90% tokens** (space+case). **d24 A/B DONE (2026-06-26): a WASH** — surface-only CORE **0.2654** ties vanilla baseline **0.2604** (10W/10L/2T, p≈1.0) with val_bpb slightly *worse* (0.71804 vs 0.71590), and below force_merges **0.2758**; the +4.90% compression bought nothing downstream (compression ≠ capability at d24/λ=0.5/single-seed; surface-only checkpoint lost in the download). The earlier fm+space+case **TRIPLE** A/B (surface ties fm) is separate **DEPRECATED** history. **λ-screen (2026-06-27, cooled K=1850, paired, on a RunPod network volume): HOLD** — λ=0.5→0.1 has **no detectable MC-trio effect** (Δtrio −0.0006 ≈ 0.08σ; per-task deltas 1–2 examples); the content-bpb gain (−0.0016) did not reach the trio (CORE-decoupled). Open: a single-task **boolq** swing (+266Q at λ=0.1, ~9σ, single seed — numbers via `runs/_lambda_screen_breakdown.py`). **PAIRED A/Bs s7 (ClimbMix) + s8 (FineWeb) + verification controls (2026-06-28): NOT a capability win.** Aggregate CORE leans +0.0072 (s7) / +0.0111 (s8), but the drivers don't replicate (s7 = commonsense_qa+boolq; s8 = copa+arc, boolq collapsed) and surf-vs-base controls account for them as artifacts: commonsense_qa = letter-prior (choice-shuffle gap collapses ~84% both seeds), boolq = surf≈base (passage-swap), rare-entity cloze = mostly the cap-bit/case mechanism + small rarity (2×2 control + CI). Robust surface effects = the by-design factored-axis bpb discounts only, not aggregate CORE/reading; the FineWeb corpus swap did not favor surface. Full writeup: idea-doc Caveats + `results/d24_surf_fw_lam0p1_s8/AB_RESULT.md`. |
