...
| [Surface factoring](ideas/surface_factoring/README.md) | ✓ | offline harness only — `tools/{space_factor_compression,case_factor_compression,case_factor_analysis,digitfix_remeasure,stack_fm_spaceless,stack_triple}.py` (tokenizer A/Bs + lossless round-trip). Model overlay (joint surface head) + harness (bit targets in dataloader/loss) NOT built. | — (no overlay yet) | **offline only — triple `force_merges + space + case` = +11.11% tokens vs baseline (space +3.08%, case +2.06%, all orthogonal/composable), 2000/2000 docs lossless; downstream CORE/bpb A/B is the gate, pending. See [ideas/surface_factoring/README.md](ideas/surface_factoring/README.md).** |
