...
| [Surface factoring](ideas/surface_factoring/README.md) | ✓ | offline tools (`tools/{space_factor_compression,case_factor_compression,case_factor_analysis,digitfix_remeasure,stack_fm_spaceless,stack_triple}.py`) PLUS full overlay + harness BUILT: `overlay/surface_{factoring,codec,tokenizer,dataloader,context}.py` + `wrappers/{train_surface_factoring,tok_train_surface}.py` + `runs/runcpu_surface.sh`; 25 smoke checks (`wrappers/smoke_surface_factoring{,_eval,_overlay,_harness}.py`). | d6 runcpu (mechanism + bpb A/B) | **offline +11.11% tokens; FIRST d6 model A/B (same commit dc54a1a, 5000 iters): surface val bpb = `1.1378` = LOWEST of any d6 run (−0.0276 vs baseline 1.1654, −0.0212 vs force_merges 1.1590), byte-basis verified apples-to-apples. ~4× force_merges' bpb gain (which → CORE +5.81%). CORE itself UNMEASURED (surface-aware eval path TODO) — the GPU A/B gate. See [ideas/surface_factoring/README.md](ideas/surface_factoring/README.md).** |
