...
| [Surface factoring](ideas/surface_factoring/README.md) | ✓ | **surface-only** (space+case on the baseline vocab, 32768, K_max=1, no force_merges) BUILT: offline tools (`tools/{space_factor_compression,case_factor_compression,case_factor_analysis}.py`) + overlay/harness `overlay/surface_{factoring,codec,tokenizer,dataloader,context,lambda}.py` + `wrappers/{train_surface_factoring,tok_train_surface}.py` + `runs/runcpu_surface.sh`; 30 smoke checks (`wrappers/smoke_surface_factoring{,_eval,_overlay,_core}.py` + `wrappers/smoke_surface_harness.py`). (`tools/stack_triple` builds the DEPRECATED triple — kept for reference only.) | **s7+s8 PAIRED A/Bs (λ=0.1) + verification controls DONE (2026-06-28): NOT a capability win** + d24 A/B DONE (2026-06-26): a WASH [superseded by paired s7/s8] + λ-screen DONE (2026-06-27): HOLD [superseded by paired s7/s8] | surface-only offline **+4.90% tokens** (space+case). **PAIRED A/Bs at λ=0.1, s7 (ClimbMix) + s8 (FineWeb) + verification controls (2026-06-28): NOT a capability win.** Aggregate CORE leans +0.0072 (s7, surf 0.2744 vs base 0.2673) / +0.0111 (s8, surf 0.2436 vs base 0.2325) and val_bpb is slightly *worse* (s8 +0.0010 train-min / +0.0011 eval); the drivers differ by seed (s7 = boolq + commonsense_qa; s8 = copa + commonsense_qa + arc, boolq collapsed); the sign-flipping movers are copa (s7 −0.060 → s8 +0.100) and boolq (s7 +0.092 → s8 −0.013), and the one cross-seed top-driver, commonsense_qa, is letter-prior in both (arc is also positive both seeds, content-scored). surf-vs-base controls account for the leans as artifacts: commonsense_qa = letter-prior (choice-shuffle gap collapses ~84% both seeds), boolq = surf≈base (passage-swap), rare-entity cloze = mostly the cap-bit/case mechanism + small rarity (2×2 control + CI); copa = small-N noise (N=100, ~41% of the s8 headline); arc = content-scored (position-invariant under choice-shuffle) and surface-positive on both seeds — the premise-swap reading control (`wrappers/content_mc_swap.py`) confirms arc accuracy is premise-dependent (reading) on the clean baseline arm, but the surface arm was noisier so the surface arc delta itself (single-corpus per seed) is not yet decomposed to an artifact. (controls: choice-shuffle `wrappers/choice_shuffle.py`; passage-swap `wrappers/boolq_swap.py`; cloze 2×2 `wrappers/rare_entity_cloze.py --control`; robustness/factored-axis `wrappers/surface_robustness.py`; FineWeb swap `wrappers/fineweb_holdout.py`; long-tail `wrappers/longtail_nll.py`; premise/stem swap `wrappers/content_mc_swap.py`; register overlap `tools/register_overlap.py`; shared scorer `wrappers/_mc_scoring.py`; driven by `runs/eval_suite.sh` / `runs/runpod_ab.sh`.) Robust surface effects = the by-design factored-axis bpb discounts only, not aggregate CORE/reading; the FineWeb corpus swap did not favor surface. Throughput: surface trains ~1.4× slower wall-clock (s8 MFU 37.28% / 181.73m vs baseline 49.25% / 131.51m; `base-model-training.md`). Full writeup: idea-doc Caveats + `results/d24_surf_fw_lam0p1_s8/AB_RESULT.md` (premise-swap/register outputs under `results/d24_surf_fw_lam0p1_s8/premise_swap/`). **Superseded context:** **d24 A/B (2026-06-26): a WASH** [superseded by the paired s7/s8 A/Bs] — λ=0.5 single-seed surface-only CORE **0.2654** ≈ vanilla baseline **0.2604** (10W/10L/2T, p≈1.0), val_bpb slightly worse (0.71804 vs 0.71590), below force_merges **0.2758** (full writeup in idea-doc Results; surface-only checkpoint lost in the download). The earlier fm+space+case **TRIPLE** A/B (surface ties fm) is separate **DEPRECATED** history. **λ-screen (2026-06-27, cooled K=1850, paired): HOLD** [superseded by the paired s7/s8 A/Bs] — λ=0.5→0.1 no detectable MC-trio effect (Δtrio −0.0006 ≈ 0.08σ). The earlier λ-screen boolq swing (+266Q at λ=0.1) is accounted for by the paired passage-swap control (`wrappers/boolq_swap.py`): surf≈baseline on both seeds, neither beats the 0.616 subset majority → not a surface-specific reading effect. (The within-λ k1850 λ0.5-vs-λ0.1 swing remains separately unprobed.) |
