...
#                + install Rust + build the rustbpe_force_merges crate (the
#                surface arm imports it via tools/stack_triple)
...
#   verify     → preflight on the host (8 GPUs, venv, both tokenizers, crate,
#                surface smoke) BEFORE the expensive run
#   train      → both arms in one tmux session, sequential, each SKIPPED if its
#                results/<tag>/eval.csv already exists:
#                  A) baseline = force_merges tokenizer, no overlay
#                  B) surface  = OVERLAY=surface_factoring + base_eval_surface
...
#
# NOTE: provisioning bills ~$16/hr (A100 40GB); a 2-arm d24 A/B is ~13-14 GPU-hrs
# ≈ ~$215 + ~$18 overhead. This script will not terminate before artifacts are
# verified local, so a crash leaves the instance up (resume, don't re-provision).
...
# --- A/B arm config ---------------------------------------------------------
BASE_TAG="d24_force_merges_ab"          # baseline = the ADOPTED force_merges tokenizer
SURF_TAG="d24_surface_factoring_ab"     # variant  = force_merges + surface factoring
...
stage_provision() {
    ...
    confirm "Launch $LAMBDA_INSTANCE_TYPE (~\$16/hr, ~\$215+ for the 2-arm d24 A/B)?" || die "aborted"
...
stage_bootstrap() {
    ...
    if _ssh "test -f ~/.ab_setup_done" 2>/dev/null; then
        log "host already bootstrapped (~/.ab_setup_done present) — skipping setup.sh + crate build"
        return 0
    fi
    log "bootstrapping host (rsync sandbox + setup.sh + wandb login)"
    bash "$LAMBDA" bootstrap "$INSTANCE_ID"
    log "installing Rust toolchain + building rustbpe_force_merges (surface arm needs it)"
    _ssh 'command -v cargo >/dev/null 2>&1 || curl --proto "=https" --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y'
    _ssh 'export PATH="$HOME/.local/bin:$HOME/.cargo/bin:$PATH" && source "$HOME/.cargo/env" 2>/dev/null; cd ~/sandbox && VARIANT=force_merges RELEASE=1 bash runs/build_rustbpe.sh'
...
stage_verify() {
    ...
    _ssh 'set -e; export PATH="$HOME/.local/bin:$HOME/.cargo/bin:$PATH"; cd ~/sandbox
        ...
        uv run python -c "import rustbpe_force_merges" || { echo "FAIL: rustbpe_force_merges not importable"; exit 1; }
        echo "--- surface smoke ---"; uv run python -m wrappers.smoke_surface_factoring 2>&1 | tail -1
        echo "PREFLIGHT OK"'
...
if [ -f results/$BASE_TAG/eval.csv ]; then
  echo "[ab] arm A ($BASE_TAG) already done — skipping"
else
  echo "[ab] arm A: baseline force_merges"
...
if [ -f results/$SURF_TAG/eval.csv ]; then
  echo "[ab] arm B ($SURF_TAG) already done — skipping"
else
  echo "[ab] arm B: surface factoring"
...
stage_poll() {
    ...
    while true; do
        ...
        local prog; prog=$(_ssh "grep -hoE 'step [0-9]+/[0-9]+' ~/sandbox/runs/${BASE_TAG}.log ~/sandbox/runs/${SURF_TAG}.log 2>/dev/null | tail -1" || true)
...
stage_download() {
    ...
    for pair in "$FM_VARIANT:$BASE_TAG" "$SURF_VARIANT:$SURF_TAG"; do
        local v="${pair%%:*}" tag="${pair##*:}"
        local rmt="~/.cache/nanochat-variants/$v/base_checkpoints/$tag/"
...
stage_verifydl() {
    ...
    for tag in "$BASE_TAG" "$SURF_TAG"; do
    ...
    log "all artifacts present locally. A/B CORE comparison is in results/{$BASE_TAG,$SURF_TAG}/."
    return 0
}
